"""
You are given an array of strings arr. Your task is to construct 
a string from the words in arr, starting with the 0th character 
from each word (in the order they appear in arr),
followed by the 1st character, then the 2nd character, etc.
...
For arr = ["Daisy", "Rose", "Hyacinth", "Poppy"],
...
def construct_string_from_words(words):
    res=""
    j=0
    ...
    while j  <mx:
        for i,word in enumerate (words):
            if j < len(word) :  
                res+=word[j]
        j+=1
    return res =="DRHPaoyoisapsecpyiynth"

words = ["Daisy", "Rose", "Hyacinth", "Poppy"]
#"DRHPaoyoisapsecp yiy nth".
print(construct_string_from_words(words))